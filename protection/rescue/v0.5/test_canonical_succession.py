...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="uu-aap-rescue-v05-test-") as td:
        ...
        predecessor = S.bind_predecessor(
            "Matawaka/uu-aap", "https://github.com/Matawaka/uu-aap", source_repo,
            "refs/heads/main", "2026-08-23T15:10:00Z",
        )
        ...
        source_binding = {
            "artifact_type": "RecoverySourceBinding", "artifact_version": "0.4",
            "source_id": "source-bundle-v05", "source_kind": "git_bundle",
            "payload_sha256": bundle_sha, "frontier_commit": frontier,
            "failure_domain_id": "offline-media:test-v05", "verified": True,
            "claims": {"canonical": False, "authority_transfer": False, "network_source": False, "truth_certified": False},
        }
        ...
        sealed_receipt = T.E.verify_recovery(recovery)
        assert sealed_receipt["recovered_frontier_commit"] == frontier
        sealed_ref_digest = sealed_receipt["recovered_ref_set_sha256"]

        # Candidate work is deliberately separated from the sealed v0.4 recovery.
        candidate_repo = t / "candidate.git"
        run(["git", "init", "--bare", str(candidate_repo)])
        run([
            "git", "-C", str(candidate_repo), "fetch", "--no-write-fetch-head",
            str((recovery / "repository.git").resolve()),
            "+refs/heads/*:refs/heads/*", "+refs/tags/*:refs/tags/*",
        ])
        assert run(["git", "-C", str(candidate_repo), "remote"]).stdout.strip() == ""
        tree = run(["git", "-C", str(candidate_repo), "rev-parse", f"{frontier}^{{tree}}"]).stdout.strip()
        candidate = run(
            ["git", "-C", str(candidate_repo), "commit-tree", tree, "-p", frontier, "-m", "local post-recovery candidate"],
            env={
                "GIT_AUTHOR_NAME": "UU-AAP Test", "GIT_AUTHOR_EMAIL": "test@example.invalid",
                "GIT_COMMITTER_NAME": "UU-AAP Test", "GIT_COMMITTER_EMAIL": "test@example.invalid",
                "GIT_AUTHOR_DATE": "2026-08-23T15:35:00Z", "GIT_COMMITTER_DATE": "2026-08-23T15:35:00Z",
            },
        ).stdout.strip()
        run(["git", "-C", str(candidate_repo), "update-ref", "refs/heads/rescue-candidate", candidate])

        proposal = S.create_proposal(
            policy, predecessor, recovery, candidate_repo, "refs/heads/rescue-candidate",
            "human:test-proposer", "2026-08-23T15:40:00Z",
        )
        ...
        assessed = S.assess_proposal(
            policy, predecessor, recovery, candidate_repo, proposal, "2026-08-23T15:45:00Z"
        )
        ...
        rejected = S.assess_proposal(
            policy, predecessor, recovery, candidate_repo, tampered, "2026-08-23T15:46:00Z"
        )
        ...
        expect_fail(
            lambda: S.create_proposal(
                policy, bad_predecessor, recovery, candidate_repo, "refs/heads/rescue-candidate",
        ...
        expect_fail(
            lambda: S.create_proposal(
                policy, predecessor, recovery, candidate_repo, "refs/tags/synthetic-predecessor",
                "human:test-proposer", "2026-08-23T15:47:00Z",
            ), "refs/heads",
        )

        run(["git", "-C", str(candidate_repo), "remote", "add", "forbidden-test-remote", "/tmp/nonexistent"])
        expect_fail(
            lambda: S.create_proposal(
                policy, predecessor, recovery, candidate_repo, "refs/heads/rescue-candidate",
        ...
        run(["git", "-C", str(candidate_repo), "remote", "remove", "forbidden-test-remote"])

        stable_proposal = S.create_proposal(
            policy, predecessor, recovery, candidate_repo, "refs/heads/rescue-candidate",
            "human:test-proposer", "2026-08-23T15:49:00Z",
        )
        run(["git", "-C", str(candidate_repo), "update-ref", "refs/heads/later-local-ref", candidate])
        drifted = S.assess_proposal(
            policy, predecessor, recovery, candidate_repo, stable_proposal, "2026-08-23T15:50:00Z"
        )
        assert drifted["state"] == "rejected"
        assert drifted["checks"]["candidate_ref_set_match"] is False

        root_commit = run(
            ["git", "-C", str(candidate_repo), "commit-tree", tree, "-m", "unrelated root"],
            env={
                "GIT_AUTHOR_NAME": "UU-AAP Test", "GIT_AUTHOR_EMAIL": "test@example.invalid",
                "GIT_COMMITTER_NAME": "UU-AAP Test", "GIT_COMMITTER_EMAIL": "test@example.invalid",
                "GIT_AUTHOR_DATE": "2026-08-23T15:51:00Z", "GIT_COMMITTER_DATE": "2026-08-23T15:51:00Z",
            },
        ).stdout.strip()
        run(["git", "-C", str(candidate_repo), "update-ref", "refs/heads/unrelated", root_commit])
        expect_fail(
            lambda: S.create_proposal(
                policy, predecessor, recovery, candidate_repo, "refs/heads/unrelated",
        ...
        # Candidate evolution never mutates the sealed recovery evidence.
        final_receipt = T.E.verify_recovery(recovery)
        assert final_receipt["recovered_ref_set_sha256"] == sealed_ref_digest
