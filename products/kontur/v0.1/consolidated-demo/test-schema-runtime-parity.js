...
assert.deepStrictEqual([...inputSchema.$defs.metrics.required].sort(), [...Runtime.METRIC_KEYS].sort(), 'metric schema/runtime drift');
assert.deepStrictEqual([...inputSchema.$defs.controls.required].sort(), [...Runtime.CONTROL_KEYS].sort(), 'control schema/runtime drift');
...
assert.strictEqual(inputSchema.$defs.metrics.properties.measurement_class.const, 'synthetic_demo_metrics');
