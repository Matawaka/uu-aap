...
function hash(v,core=false){
  const p={}; for(const [k,x] of Object.entries(v)){if(k==="content_hash"||(core&&k==="signature_profile"))continue;p[k]=x}
  return `sha256:${crypto.createHash("sha256").update(Buffer.from(canon(p),"utf8")).digest("hex")}`;
}
function t(v,l){const n=Date.parse(v);if(!Number.isFinite(n))fail(`${l} must be timestamp`);return n}
function falses(v,keys,l){if(!obj(v))fail(`${l} must be object`);for(const k of keys)if(v[k]!==false)fail(`${l}.${k} must be exactly false`)}
function sameRef(a,b){return obj(a)&&obj(b)&&a.descriptor_id===b.descriptor_id&&a.content_hash===b.content_hash}
const OBS_FALSE=["intent_established","authority_granted","approval_created","action_permit_created","action_performed","future_availability_guaranteed","outcome_success_guaranteed","truth_certified","liability_established"];
const TOP_FALSE=["intent_established","authority_granted","approval_created","action_permit_created","action_authorized","action_performed","future_availability_guaranteed","outcome_success_guaranteed","causality_proven","truth_certified","liability_established","future_action_permission_created"];

function stateOK(s,o){
  if(!obj(s)||s.protocol!=="UU-AAP Core"||s.version!=="0.1"||s.receipt_type!=="StateReceipt")fail("core_state_receipt must be Core v0.1 StateReceipt");
  ...
  falses(s.non_effects,["intent_established","authority_established","action_performed","liability_established","truth_certified"],"StateReceipt.non_effects");
  if(t(s.frontier.observed_at,"StateReceipt.observed_at")>t(o.frontier.observed_at,"observation.observed_at"))fail("StateReceipt cannot observe after availability observation");
  const h=hash(s,true); if(s.content_hash!==h)fail(`StateReceipt content hash mismatch: expected ${h}`);
}
function claimOK(c,s,r,o){
  if(!obj(c)||c.protocol!=="UU-AAP Core"||c.version!=="0.1"||c.receipt_type!=="AvailabilityClaim")fail("core_availability_claim must be Core v0.1 AvailabilityClaim");
  ...
  if(!Array.isArray(c.predecessor_receipt_hashes)||c.predecessor_receipt_hashes.length!==1||c.predecessor_receipt_hashes[0]!==s.content_hash)fail("AvailabilityClaim must bind exactly the Core StateReceipt predecessor");
  ...
  const observed=t(o.frontier.observed_at,"observation.observed_at"),issued=t(c.issued_at,"AvailabilityClaim.issued_at"),until=t(o.valid_until,"observation.valid_until");
  ...
  const p=c.payload;if(!obj(p)||p.status!=="available")fail("positive AvailabilityClaim payload.status must be available");
  ...
  const h=hash(c,true);if(c.content_hash!==h)fail(`AvailabilityClaim content hash mismatch: expected ${h}`);
}
function validate(r){
  if(!obj(r)||r.protocol!=="UU-AAP-EXECUTION-CAPABILITY-AVAILABILITY"||r.version!=="0.1"||r.artifact_type!=="ExecutionCapabilityAvailabilityBindingRecord")fail("record identity mismatch");
  const sf=JSON.parse(fs.readFileSync(SELECTION,"utf8")),b=r.selection_binding;
  ...
  if(sf.result.status!=="selected")fail("selection fixture must be selected");
  ...
  const started=t(o.probe.started_at,"probe.started_at"),completed=t(o.probe.completed_at,"probe.completed_at"),observed=t(o.frontier.observed_at,"observation.observed_at"),until=t(o.valid_until,"observation.valid_until");
  ...
  const req=o.probe.required_check_ids,checks=o.probe.checks;
  ...
  const by=new Map();for(const c of checks){if(by.has(c.check_id))fail("duplicate availability check");by.set(c.check_id,c)}
  ...
  }else fail("unknown availability status");
  falses(o.non_effects,OBS_FALSE,"observation.non_effects");
  const oh=hash(o);if(o.content_hash!==oh)fail(`observation content hash mismatch: expected ${oh}`);
  ...
  if(r.assertions.selection_binding_verified!==true||r.assertions.state_frontier_preserved!==true||r.assertions.positive_availability_only_if_all_required_checks_pass!==true)fail("binding assertions incomplete");
  falses(r.non_effects,TOP_FALSE,"record.non_effects");
  const rh=hash(r);if(r.content_hash!==rh)fail(`record content hash mismatch: expected ${rh}`);
}
function clone(v){return JSON.parse(JSON.stringify(v))}
function rehash(r){
  r.observation.content_hash=hash(r.observation);r.core_state_receipt.content_hash=hash(r.core_state_receipt,true);
  if(r.core_availability_claim){
    if(r.core_availability_claim.predecessor_receipt_hashes.length===1)r.core_availability_claim.predecessor_receipt_hashes[0]=r.core_state_receipt.content_hash;
    if(obj(r.core_availability_claim.payload))r.core_availability_claim.payload.availability_observation_hash=r.observation.content_hash;
    r.core_availability_claim.content_hash=hash(r.core_availability_claim,true);
  }
  r.content_hash=hash(r);
}
const schema=JSON.parse(fs.readFileSync(SCHEMA,"utf8"));if(schema.title!=="UU-AAP Execution Capability Availability Binding v0.1")fail("schema title mismatch");
const cs=JSON.parse(fs.readFileSync(CORE_SCHEMA,"utf8"));if(cs.title!=="UU-AAP Core v0.1 Receipt Envelope"||!cs.properties.receipt_type.enum.includes("AvailabilityClaim"))fail("Core AvailabilityClaim contract missing");
const fixture=JSON.parse(fs.readFileSync(FIXTURE,"utf8"));validate(fixture);process.stdout.write(`PASS positive: ${fixture.binding_id}\n`);
function neg(name,fn,re,raw=false){const v=clone(fixture);fn(v);if(!raw)rehash(v);try{validate(v)}catch(e){if(!re.test(String(e.message)))fail(`${name} wrong failure: ${e.message}`);process.stdout.write(`PASS negative: ${name}\n`);return}fail(`${name} unexpectedly passed`)}
