...
async function main(){
  ...
  vectors.push(await reject('claim_semantic_frontier_drift',async()=>{const x=clone(notYet);x.responsibility_event_head.semantic_binding.action='other.action';await Ingress.validateClaim({claim:x,frontierEntry:frontier,policy});},/frontier substitution/));
  vectors.push(await reject('claim_effect_frontier_drift',async()=>{const x=clone(notYet);x.responsibility_event_head.effect_frontier.tree_sha='0'.repeat(40);await Ingress.validateClaim({claim:x,frontierEntry:frontier,policy});},/frontier substitution/));
