...
function validateArtifactBindings(items, releaseMap, expectedRole, label) {
  ...
  for (const item of items) {
    ...
    if (expectedRole) {
      assert(releaseArtifact.role === expectedRole, `${label}:${item.path}: expected release role ${expectedRole}, got ${releaseArtifact.role}`);
    }
...
function verifyAttestation(attestation, options = {}) {
  ...
  exactKeys(attestation, [
    '$schema', 'artifact_type', 'artifact_version', 'attestation_id', 'scope', 'subject',
    'protocol_binding', 'dependency_artifacts', 'attested_conformance_levels', 'test_evidence',
    'verification_policy', 'non_claims'
  ], 'attestation');
  ...
  assert(typeof attestation.subject.subject_id === 'string' && attestation.subject.subject_id.length > 0, 'subject: missing subject_id');

  const binding = attestation.protocol_binding;
  exactKeys(binding, [
    'registry_id', 'protocol_id', 'version', 'logical_uri', 'release_tag', 'release_commit',
    'release_manifest_path', 'release_manifest_git_blob_sha'
  ], 'protocol_binding');
  ...
  assert(actualCommit === binding.release_commit, `protocol_binding: tag moved; expected ${binding.release_commit}, got ${actualCommit}`);
  ...
  assert(releaseManifest.protocol.id === binding.protocol_id, 'release manifest: protocol mismatch');
  assert(releaseManifest.protocol.version === binding.version, 'release manifest: version mismatch');
  ...
  validateArtifactBindings(attestation.subject.artifact_set, releaseArtifacts, 'reference_implementation', 'subject.artifact_set');
  validateArtifactBindings(attestation.dependency_artifacts, releaseDependencies, null, 'dependency_artifacts');
  ...
  for (const evidence of attestation.test_evidence) {
    ...
    assert(safePath(evidence.test_path), `test_evidence:${evidence.level}: unsafe test path`);
    assert(sha1(evidence.git_blob_sha), `test_evidence:${evidence.level}: invalid test blob SHA`);
    ...
    assert(releaseTest, `test_evidence:${evidence.level}: test is not bound by immutable release manifest`);
    assert(releaseTest.role === 'conformance_test', `test_evidence:${evidence.level}: release artifact is not a conformance_test`);
    assert(releaseTest.conformance.includes(evidence.level), `test_evidence:${evidence.level}: release test does not claim this level`);
    ...
    assert(Array.isArray(evidence.runner.args) && evidence.runner.args.length === 1, `test_evidence:${evidence.level}: runner must have exactly one argument`);
    assert(evidence.runner.args[0] === evidence.test_path, `test_evidence:${evidence.level}: command substitution detected`);

    if (rerunTests) {
      const run = cp.spawnSync('node', [evidence.test_path], {
        cwd: repoRoot,
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'pipe']
      });
      if (run.error) fail(`test_evidence:${evidence.level}: execution error: ${run.error.message}`);
      assert(run.status === 0, `test_evidence:${evidence.level}: test failed with exit ${run.status}\n${run.stdout || ''}\n${run.stderr || ''}`);
  ...
  exactKeys(attestation.verification_policy, [
    'rerun_required', 'implementation_blob_match_required', 'dependency_blob_match_required',
    'test_blob_match_required', 'release_manifest_membership_required', 'command_binding'
  ], 'verification_policy');
  assert(attestation.verification_policy.rerun_required === true, 'verification_policy: rerun_required must be true');
  assert(attestation.verification_policy.implementation_blob_match_required === true, 'verification_policy: implementation blob matching required');
  assert(attestation.verification_policy.dependency_blob_match_required === true, 'verification_policy: dependency blob matching required');
  assert(attestation.verification_policy.test_blob_match_required === true, 'verification_policy: test blob matching required');
  assert(attestation.verification_policy.release_manifest_membership_required === true, 'verification_policy: release manifest membership required');
  assert(attestation.verification_policy.command_binding === 'node_single_test_path_exact', 'verification_policy: unsupported command binding');
  ...
  for (const claim of [
    'production_deployment_identity_established',
    'universal_operational_capability_established',
    'legal_authority_established',
    'execution_admission_established',
    'materialization_permission_established'
  ]) assert(attestation.non_claims.includes(claim), `non_claims: required boundary missing ${claim}`);
...
if (require.main === module) {
  ...
  const result = verifyAttestation(readJson(file), { rerunTests: true });
