...
const ORIGIN = { repository:'Matawaka/uu-aap', revision:'916394efdd51d845d695b02d7e11543a1426bb30', tree:'cefdec0c4a9434ab2f66575e4fb61aaaa6336cb7' };
...
function testPositiveInterruptCoordination(){const input=inputFor(),r=buildCoreCoordinationReceipt(input);assert.strictEqual(r.receipt_type,'CoordinationReceipt');assert.strictEqual(r.assertions.coordination_established,true);assert.strictEqual(r.assertions.coordination_scope,'fcl.run.interrupt');assert.strictEqual(r.assertions.availability_fresh_at_coordination,true);assert.strictEqual(r.frontier.observed_at,input.issued_at);assert.deepStrictEqual(r.predecessor_receipt_hashes,[input.core_availability_claim.content_hash,input.core_intent_receipt.content_hash,input.core_authority_receipt.content_hash]);assert.strictEqual(validateBoundCoordinationReceipt(r,input),true);}
...
function testAvailabilityBindingContainsNoIntent(){const input=inputFor();assert.deepStrictEqual(Object.keys(input.core_availability_claim.payload.fcl_binding).sort(),[...AVAILABILITY_BINDING_KEYS].sort());assert.strictEqual('intent_ref' in input.core_availability_claim.payload.fcl_binding,false);assert.strictEqual('requested_control' in input.core_availability_claim.payload.fcl_binding,false);}
...
function testAvailabilityEpochBindingMismatch(){const input=inputFor();input.core_availability_claim.payload.fcl_binding.run_epoch+=1;rehash(input.core_availability_claim);expectFailure('availability epoch binding',()=>validateInput(input),/run_epoch/);}
function testAvailabilityChainBindingMismatch(){const input=inputFor();input.core_availability_claim.payload.fcl_binding.chain_id='different-chain';rehash(input.core_availability_claim);expectFailure('availability chain binding',()=>validateInput(input),/chain_id/);}
function testAvailabilityScopeBindingMismatch(){const input=inputFor();input.core_availability_claim.payload.fcl_binding.operation_scope='fcl.run.successor.create';rehash(input.core_availability_claim);expectFailure('availability scope binding',()=>validateInput(input),/operation_scope/);}
function testAvailabilityTargetBindingMismatch(){const input=inputFor();input.core_availability_claim.payload.fcl_binding.target='urn:uu-aap:fcl:run:*';rehash(input.core_availability_claim);expectFailure('availability target binding',()=>validateInput(input),/target/);}
function testAvailabilityIntentLeakRejected(){const input=inputFor();input.core_availability_claim.payload.fcl_binding.intent_ref=input.fcl_authority_evaluation.intent_ref;rehash(input.core_availability_claim);expectFailure('availability intent leak',()=>validateInput(input),/keys mismatch/);}
function testAvailabilityControlLeakRejected(){const input=inputFor();input.core_availability_claim.payload.fcl_binding.requested_control=input.fcl_authority_evaluation.requested_control;rehash(input.core_availability_claim);expectFailure('availability control leak',()=>validateInput(input),/keys mismatch/);}
function testAvailabilityStatusMustBeAvailable(){const input=inputFor();input.core_availability_claim.payload.status='unavailable';rehash(input.core_availability_claim);expectFailure('availability status',()=>validateInput(input),/status must be available/);}
function testAvailabilityValidUntilRequired(){const input=inputFor();delete input.core_availability_claim.payload.valid_until;rehash(input.core_availability_claim);expectFailure('availability valid_until',()=>validateInput(input),/valid_until required/);}
function testAvailabilityClaimIssuedAfterExpiryRejected(){const input=inputFor();input.core_availability_claim.payload.valid_until='2026-08-27T17:59:58Z';rehash(input.core_availability_claim);expectFailure('availability issued after expiry',()=>validateInput(input),/freshness window|issued after availability expired/);}
function testAvailabilityStaleAtCoordinationRejected(){const input=inputFor();input.core_availability_claim.payload.valid_until='2026-08-27T18:01:02Z';rehash(input.core_availability_claim);expectFailure('availability stale at coordination',()=>validateInput(input),/stale at coordination/);}
...
function testCoordinationObservedAtDecisionTime(){const input=inputFor(),r=buildCoreCoordinationReceipt(input);assert.strictEqual(r.frontier.revision,input.core_state_receipt.frontier.revision);assert.strictEqual(r.frontier.observed_at,input.issued_at);}
...
function testOutputObservedAtSubstitutionRejected(){const input=inputFor(),r=buildCoreCoordinationReceipt(input);r.frontier.observed_at='2026-08-27T18:01:04Z';rehash(r);expectFailure('coordination observed_at',()=>validateBoundCoordinationReceipt(r,input),/observed_at/);}
function testAvailabilityHorizonExtensionRejected(){const input=inputFor(),r=buildCoreCoordinationReceipt(input);r.payload.availability_valid_until='2026-08-27T19:00:00Z';rehash(r);expectFailure('availability horizon',()=>validateBoundCoordinationReceipt(r,input),/availability_valid_until mismatch/);}
...
function testExecutionAdmissionOverclaimRejected(){const input=inputFor(),r=buildCoreCoordinationReceipt(input);r.non_effects.execution_admitted=true;rehash(r);expectFailure('execution admission overclaim',()=>validateBoundCoordinationReceipt(r,input),/execution_admitted/);}
...
function testAuthorityCreationOverclaimRejected(){const input=inputFor(),r=buildCoreCoordinationReceipt(input);r.non_effects.authority_created=true;rehash(r);expectFailure('authority creation overclaim',()=>validateBoundCoordinationReceipt(r,input),/authority_created/);}
...
function testAvailabilityDoesNotCreateIntentOrAuthority(){const input=inputFor();assert.strictEqual(input.core_availability_claim.non_effects.intent_established,false);assert.strictEqual(input.core_availability_claim.assertions.capability,input.fcl_authority_evaluation.required_scope);const r=buildCoreCoordinationReceipt(input);assert.strictEqual(r.non_effects.authority_granted,false);assert.strictEqual(r.non_effects.availability_extended,false);}
...
function run(){
  const tests=[testPositiveInterruptCoordination,testPositiveSuccessorCoordination,testDeterministicAndCoreHashParity,testClosedMiniChainEdges,testAvailabilityBindingContainsNoIntent,testStatePredecessorRejected,testStateAnchoredRequired,testStateNonEffectRequired,testAvailabilityPredecessorRejected,testAvailabilitySubjectMismatch,testAvailabilityFrontierMismatch,testAvailabilityCapabilityMismatch,testAvailabilityBindingMissing,testAvailabilityRunBindingMismatch,testAvailabilityEpochBindingMismatch,testAvailabilityChainBindingMismatch,testAvailabilityScopeBindingMismatch,testAvailabilityTargetBindingMismatch,testAvailabilityIntentLeakRejected,testAvailabilityControlLeakRejected,testAvailabilityStatusMustBeAvailable,testAvailabilityValidUntilRequired,testAvailabilityClaimIssuedAfterExpiryRejected,testAvailabilityStaleAtCoordinationRejected,testIntentPredecessorRejected,testIntentSubjectMismatch,testIntentFrontierMismatch,testAuthorityMustMatch547Binding,testAuthorityPredecessorRejected,testNonPositiveFCLRejected,testTamperedFCLFingerprintRejected,testAvailabilityIssuedBeforeStateRejected,testIntentIssuedBeforeStateRejected,testCoordinationIssuedBeforeAuthorityRejected,testCoordinationObservedAtDecisionTime,testOutputPredecessorSubstitutionRejected,testOutputSubjectSubstitutionRejected,testOutputFrontierSubstitutionRejected,testOutputObservedAtSubstitutionRejected,testAvailabilityHorizonExtensionRejected,testExecutionOverclaimRejected,testExecutionAdmissionOverclaimRejected,testActionPermitOverclaimRejected,testAuthorityCreationOverclaimRejected,testLegalAuthorityOverclaimRejected,testReceiptTypeEscalationRejected,testAvailabilityDoesNotCreateIntentOrAuthority,testReadOnlyCli];
  for(const test of tests){test();process.stdout.write(`PASS ${test.name}\n`);}process.stdout.write(`PASS FCL Core CoordinationReceipt Binding v0.1 reconciled conformance (${tests.length} groups)\n`);
