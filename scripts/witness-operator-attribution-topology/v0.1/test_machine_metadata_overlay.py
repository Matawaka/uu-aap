...
for x in p['operators']:
 recs.append({'operator_label':x['operator_label'],'attribution_url':x['attribution_url'],'key_material_url':o.MON if x['operator_label']=='Geomys' else x['key_material_url'],'registry_relation_observed':True,'operator_key_material_relation_observed':True,'all_expected_vkeys_observed':x['operator_label']!='Geomys'})
