...
def must_block(condition, case, reason):
    if condition and case["response_authorized"]:
        raise AssertionError(f"{case['id']}: must be blocked: {reason}")
...
def validate(payload):
    require(payload.get("schema_version") == "0.1", "schema_version must be 0.1")
    ...
    for case in cases:
        ...
        if case["interaction_owner"] == "PLAYER_LED":
            require(case["initiative_authorized"] is False, f"{cid}: player-led receipt cannot carry initiative authority")
            ...
            must_block(chosen > intent_max, case, "assistance exceeds player intent")
        else:
            must_block(not case["initiative_authorized"], case, "companion-led response lacks initiative authorization")
            must_block(case["initiative_depth"] == "NONE", case, "companion-led response has no initiative depth")
            if case["initiative_depth"] != "NONE":
                must_block(chosen > INITIATIVE[case["initiative_depth"]], case, "assistance exceeds proactive initiative depth")
            must_block(chosen > ASSISTANCE["NUDGE"], case, "companion-led response exceeds proactive NUDGE ceiling")
        ...
        must_block(case["player_focus_present"] and case["focus_overrides_player"], case, "system focus overrides player-selected focus")

        allowed_spoiler_level = max(case["already_revealed_spoiler_level"], case["spoiler_budget"])
        must_block(case["new_spoiler_level"] > allowed_spoiler_level, case, "spoiler disclosure exceeds budget/prior exposure")
        ...
        must_block(irreversible_uncertain_risk and chosen > ASSISTANCE["NUDGE"], case, "deep uncertain irreversible assistance")
        must_block(
            irreversible_uncertain_risk
        ...
        if case["player_correction_received"]:
            must_block(case["correction_outcome"] not in {"REVISED", "CONTESTED"}, case, "player correction not revised or contested")
        else:
        ...
        if case["dependency_risk_signal"]:
            ...
            must_block(not case["player_can_ignore"], case, "anti-dependency made response non-ignorable")

        if case["response_authorized"]:
            require(case["receipt_complete"], f"{cid}: authorized response requires complete receipt")
            require(case["agency_preserved"], f"{cid}: authorized response must preserve agency")
            require(case["player_can_ignore"], f"{cid}: authorized response must remain ignorable")
            require(case["help_refused"] is False, f"{cid}: authorized response cannot encode refusal")

    by_id = index_cases(payload)
    for cid, expected in EXPECTED_AUTHORIZATION.items():
        require(by_id[cid]["response_authorized"] is expected, f"{cid}: canonical authorization changed")

    require(by_id["explicit-hint-request"]["assistance_depth"] == "HINT", "hint case must remain HINT")
    require(by_id["bounded-current-cue-question"]["initiative_depth"] == "QUESTION", "current cue must remain bounded QUESTION")
    require(by_id["active-open-mystery-callback"]["memory_status"] == "ACTIVE", "open mystery memory must be active")
    require(by_id["active-open-mystery-callback"]["memory_scope_match"] is True, "open mystery memory must be local")
    require(by_id["player-correction-revises-local-model"]["correction_outcome"] == "REVISED", "correction must revise local model")
    require(by_id["unsolicited-deep-solution-blocked"]["assistance_depth"] == "SOLUTION", "unsolicited candidate must test SOLUTION")
    require(by_id["expired-cross-scope-memory-blocked"]["memory_status"] == "EXPIRED", "expired-memory boundary changed")
    require(by_id["predicted-interest-override-blocked"]["focus_overrides_player"] is True, "focus override boundary changed")
    require(by_id["uncertain-irreversible-proactive-nudge-blocked"]["epistemic_mode"] == "GUESS", "irreversible case must remain uncertain")
    require(by_id["dependency-signal-shallower-help"]["dependency_risk_signal"] is True, "dependency signal boundary changed")
    require(by_id["dependency-signal-shallower-help"]["assistance_depth"] == "QUESTION", "anti-dependency case must stay shallower")
...
NEGATIVE_MUTATIONS = [(f"boundary:{field}", mutate_boundary(field)) for field in sorted(BOUNDARY_FALSE)] + [
    ("authorized-incomplete-receipt", mutate_case("player-led-conversation", "receipt_complete", False)),
    ("authorized-agency-lost", mutate_case("player-led-conversation", "agency_preserved", False)),
    ("authorized-not-ignorable", mutate_case("player-led-conversation", "player_can_ignore", False)),
    ...
    ("companion-initiative-without-authority", mutate_case("bounded-current-cue-question", "initiative_authorized", False)),
    ...
    ("unsolicited-solution-authorized", mutate_case("unsolicited-deep-solution-blocked", "response_authorized", True)),
    ("expired-memory-authorized", mutate_case("expired-cross-scope-memory-blocked", "response_authorized", True)),
    ("predicted-focus-override-authorized", mutate_case("predicted-interest-override-blocked", "response_authorized", True)),
    ("irreversible-uncertain-nudge-authorized", mutate_case("uncertain-irreversible-proactive-nudge-blocked", "response_authorized", True)),
...
def main():
    ...
    print(f"KONTUR Game Companion interaction receipt: PASS ({rejected} fail-closed mutations rejected)")
